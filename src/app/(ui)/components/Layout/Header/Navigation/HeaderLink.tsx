...
const HeaderLink: React.FC<{ item: HeaderItem }> = ({ item }) => {
  ...
  return (
    ...
    >
      <Link href={item.href} className={`text-[17px] !text-primary py-3 uppercase  flex font-semibold hover:!text-secondary  ${path === item.href ? '!text-primaryPink' : '!text-primary'} ${path.startsWith(`/${item.label.toLowerCase()}`) ? '!text-primary' : null}`}>
