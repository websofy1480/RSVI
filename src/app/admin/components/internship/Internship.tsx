...
export const Internship: React.FC = () => {
  ...
  const fetchData = async () => {
    const internshipRes = await fetch("/api/auth/internship");
    const internshipJson = await internshipRes.json();
    setData(internshipJson.data);
  };
  ...
  const handleSave = async (form: Form) => {
    ...
    try {
      if (modal.mode === "create") {
        res = await fetch("/api/auth/internship", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(form)
        });
      } else if (modal.mode === "edit") {
        res = await fetch(`/api/auth/internship/${modal.item?._id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(form),
        });
  ...
  return (
    <div className="p-4">
      ...
      <PageBreadcrumb pageTitle="Internship" />
          ...
          {currentData.map((item) => (
            <tr key={item._id} className="text-center">
              ...
              >
                ...
                <span className="" onClick={() => { setInternshipDescription(item.description), setShowDescription(!showDescription) }}
                >
                  {showDescription ? (
      ...
      {
        showDescription && <MessageModel closedModel={setShowDescription} data={internshipDescription} mode="internship" />
      }
    </div>
  );
}
