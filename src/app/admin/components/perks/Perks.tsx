...
export const Perks:React.FC = () => {
  ...
  const handleSave = async (form: Form) => {
    ...
    try {
      if (modal.mode === "create") {
        res = await fetch("/api/auth/perks", {
          method: "POST",
  ...
  return (
    <div className="p-4">
      ...
      <PageBreadcrumb pageTitle="Perks" />
      ...
      {
        showDescription && <MessageModel closedModel={setShowDescription} data={perksDescription} mode="perks" />
      }
    </div>
  );
}
